...
**Why:** character-level similarity was repeatedly matching "Cardinal Heating and Air Conditioning" (Wisconsin) to "Cardinal Heating & Cooling LLC" (Arizona) and pasting the wrong LinkedIn onto the dossier. Splitting extraction from validation made that class of error structurally impossible and gave every contact a written reason that can be audited later. Implemented in `pipeline/16_tavily_contact_search.py` (pure extractor) and `pipeline/17_candidate_validator.py` (validator).
